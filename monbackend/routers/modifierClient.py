...
# from models import Consommation, Client 
...
#     tel_exists = db.query(Client).filter(
#         Client.telephone == client_data.telephone,
#         Client.id != client_id
#     ).first()
#     if tel_exists:
#         raise HTTPException(status_code=400, detail="Téléphone déjà utilisé par un autre client")

#     # Vérification unicité codeClient
#     code_exists = db.query(Client).filter(
#         Client.codeClient == client_data.codeClient,
#         Client.id != client_id
#     ).first()
#     if code_exists:
#         raise HTTPException(status_code=400, detail="Ce code client est déjà utilisé.")

#     # Mise à jour des champs classiques
#     for field, value in client_data.dict().items():
#         setattr(client, field, value)

#     # Mise à jour du mot de passe si date_naissance modifiée
#     if client.date_naissance != client_data.date_naissance:
#         client.password = hash_password(client_data.date_naissance.isoformat())
